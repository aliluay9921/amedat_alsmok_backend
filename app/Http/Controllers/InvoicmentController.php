        ...
        if (isset($_GET['query'])) {
            $invoicemnts->where(function ($q) {
                ...
                $q->whereHas("process", function ($query) {
                    $query->WhereHas('representativ', function ($query) {
                        $query->where('full_name', 'LIKE', '%' . $_GET['query'] . '%');
                    })->orwhere('place', 'LIKE', '%' . $_GET['query'] . '%')->orwhere('date', 'LIKE', '%' . $_GET['query'] . '%')->orwhere('name_customer', 'LIKE', '%' . $_GET['query'] . '%');
                });
