...
class AccountingController extends Controller
{
    ...
    public function getUsers()
    {
        ...
        if (!isset($_GET['limit']))
            $_GET['limit'] = 10;
        $res = $this->paging($users->orderBy("created_at", "desc"),  $_GET['skip'],  $_GET['limit']);
        return $this->send_response(200, 'تم جلب المتسخدمين بنجاح', [], $res["model"], null, $res["count"]);
    }
    ...
    public function resetPassword(Request $request)
    {
        ...
        $validator = Validator::make($request, [
            ...
            'user_id.required' => 'يجب اختيار المستخدم المراد تعديل كلمة مروره',
            'user_id.exists' => 'المستخدم الذي قمت بأختياره غير موجود',
            'password.required' => 'يجب ادخال كلمة مرور جديدة',
        ]);
        if ($validator->fails()) {
            return $this->send_response(401, 'خطأ بالمدخلات', $validator->errors(), []);
        }
        ...
        $user_info = UserInfo::find($request["user_id"]);
        if ($user_info) {
            $user_info->update(["password" => $request["password"]]);
        } else {
            UserInfo::create([
                "user_id" => $user->id,
                "password" => $request["password"]
        ...
        return $this->send_response(200, 'تم تعديل كلمة المرور بنجاح', [], User::find($request["user_id"]));
    }
}
